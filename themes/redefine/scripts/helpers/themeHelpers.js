...
hexo.extend.helper.register("createNewArchivePosts", function (posts) {
  ...
  postList.forEach((item) =>
    ...
    item.postList.sort(function(a, b) {
        // 根据title标题截取数字排序
        if(a.title.includes(".") && b.title.includes(".")) {
            a.top = parseInt(a.title.substring(0,a.title.indexOf(".")));
            b.top = parseInt(b.title.substring(0,b.title.indexOf(".")));
            // 两篇文章top都有定义
            if(a.top == b.top) return b.date - a.date; // 若top值一样则按照文章日期降序排
            else return a.top - b.top; // 否则按照top值升序排
        }
        ...
        //     return 1;
        // }
        else return b.date.unix() - a.date.unix(); // 都没定义按照文章日期降序排
    })
  );
  return postList;
});
